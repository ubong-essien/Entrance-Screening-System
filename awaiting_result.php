...
if($dept==-1){
    ...
    $resultset=mysqli_query($con,"SELECT pstudentinfo_tb_ar_ar.FirstName, pstudentinfo_tb_ar_ar.SurName, pstudentinfo_tb_ar_ar.OtherNames,pstudentinfo_tb_ar_ar.JambNo,pstudentinfo_tb_ar_ar.Gender,pstudentinfo_tb_ar_ar.LGA,pstudentinfo_tb_ar_ar.DOB,pstudentinfo_tb_ar_ar.StateId,pstudentinfo_tb_ar_ar.Phone, pstudentinfo_tb_ar_ar.JambAgg,pstudentinfo_tb_ar_ar.JmbComb,pstudentinfo_tb_ar_ar.OtherCert,pstudentinfo_tb_ar_ar.ProgID,recommendation_tb.ModeOfEntry,recommendation_tb.system_reason,recommendation_tb.panel_reason,recommendation_tb.panel_rec,recommendation_tb.medical_rec,recommendation_tb.panel_olvl_def,recommendation_tb.dept,recommendation_tb.final_status 
    FROM recommendation_tb 
    INNER JOIN pstudentinfo_tb_ar_ar 
    on recommendation_tb.RegNo = pstudentinfo_tb_ar_ar.JambNo WHERE recommendation_tb.final_status = {$search_criteria} AND recommendation_tb.ModeOfEntry={$modeofentry} ORDER BY dept ASC,JambAgg DESC");
   ...
   // var_dump($resultset);
    ...
    FROM recommendation_tb 
    INNER JOIN pstudentinfo_tb_ar_ar 
    on recommendation_tb.RegNo = pstudentinfo_tb_ar_ar.JambNo WHERE recommendation_tb.final_status = {$search_criteria} AND recommendation_tb.dept={$dept} AND recommendation_tb.ModeOfEntry={$modeofentry} ORDER BY dept ASC,JambAgg DESC");
...
<div class="wrapper" style="margin-top:70px;margin-left:0px;">
				...
				<h4 style="text-align:center;font-weight:bold;font-family:san-serif;">LIST OF <?php if($search_criteria==1){echo"ADMISSIBLE AWAITING RESULT ";}elseif($search_criteria==0){echo "NOT-ADMISSIBLE AWAITING RESULT ";}else{echo "SCREENED";} if($modeofentry==2){echo " DIRECT-ENTRY";}else{echo " PUTME";}?> CANDIDATES</h4>
				...
				<table width="100%">
						...
						<tr>
						<form action="awaiting_result.php" method="post">
						<td style="width:5%;">
			...
			/******************this is to get deficient subject for candidate missing a particulay subject but was selelcted by the panel************************************/
						...
						/*******************************************************************************************************************************************/
											...
											<!--	<td><?php //echo GetProgDetails($ProgID,$con);?></td>-->
												...
												if($panel_reason==2 && $def_subjects!=""){echo GetReason($con,$panel_reason)."<span style='font-family:arial;'><b>O-level Deficiency(s): ".$def_sub."</span>-".$missing_def;}else{echo "<span>".GetReason($con,$panel_reason)."(".$system_reason.") , </span>";}//$def_subjects----(</b><b style='font-family:monospace'>system:".$def_sub."</b>)---$def_sub."--".$def_subjects."--"
												?>
