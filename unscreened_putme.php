	...
	<div class="row">
			<div class="col-lg-12">
				...
				<table width="100%">
						...
						<form action="unscreened_putme.php" method="post">
						<td style="width:50%;">
    ...
    <?php
        ...
        if(!empty(mysqli_real_escape_string($con,(trim($_POST['generate']))))){
         ...
         $listtype=mysqli_real_escape_string($con,(trim($_POST['listtype'])));	
            ?>
	<div class="panel-body" >
    <h4 style="text-align:center;font-weight:bold;font-family:san-serif;">LIST OF UTME CANDIDATES(<?php if($listtype==1){echo "MEDICAL REPORT SHEET";}else if($listtype==2){echo "REMARK SHEET";}else if($listtype==3){echo "ATTENDANCE SHEET";}?>)</h4>
...
//$assigned_id=6;						
			//	echo $assigned_id;
								...
								//	$getstd=getAllRecord($con,"pstudentinfo_tb_ar","ModeOfEntry =2 AND RegLevel=6","ProgID ASC,SurName ASC","");
                                    ...
                                    $getstd= mysqli_query($con,"SELECT * FROM  pstudentinfo_tb_ar  WHERE JambNo IN(SELECT DISTINCT JambNo FROM pstudentinfo_tb_ar WHERE JambNo NOT IN(SELECT RegNo FROM recommendation_tb)) AND ModeOfEntry=1");
